#!/bin/bash
# Script to optimize for MacBook Pro M2

# Create a backup of the original docker-compose.yml
cp docker-compose.yml docker-compose.yml.bak

# Replace the docker-compose.yml file with M2-optimized version
...
if grep -q "OLLAMA_MODEL=" .env; then
  ...
  sed -i '' 's/OLLAMA_MODEL=.*/OLLAMA_MODEL=llama3:8b-instruct/' .env
else
  # Add OLLAMA_MODEL if it doesn't exist
  echo "OLLAMA_MODEL=llama3:8b-instruct" >> .env
fi

echo "Configuration has been optimized for MacBook Pro M2!"
...
echo "You can now run 'docker-compose up -d' to start the services with optimized settings."
echo ""
echo "IMPORTANT: Make sure Docker Desktop has sufficient resources allocated:"
echo "- Open Docker Desktop preferences"
echo "- Go to 'Resources'"
echo "- Allocate at least 12GB of memory"
echo "- Allocate at least 4 CPUs"
